...
{ time source ./run.sh; } |& grep "real" >> ~/spec-configs/wamrl-"$SIZE".txt
{ time source ./run.sh; } |& grep "real" >> ~/spec-configs/wamrl-"$SIZE".txt
{ time source ./run.sh; } |& grep "real" >> ~/spec-configs/wamrl-"$SIZE".txt
./check.sh | grep -v "specdiff run completed" >> ~/spec-configs/wamrl-"$SIZE".txt
echo "-------------------------" >> ~/spec-configs/wamrl-"$SIZE".txt